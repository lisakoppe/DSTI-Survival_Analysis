...
lungSCC <- mutate(lungSCC_raw,
                  ...
                  Smoking_state = factor(Smoking_state, levels = c("3", "1", "2", "0"), labels = c("Non-smoker", "Current-smoker", "Ex-smoker", "Unknown")),
...
8. Comparison between 2 groups of patients based on the type of operation undergone by the patient:
```{r}
fit.KM_OP <- survfit(Surv(OS, death) ~ OP, data=lungSCC)
fit.KM_OP
```

We plot the Kaplan-Meier survival curve:
```{r}
gg <- ggsurvplot(fit.KM_OP,
           legend = "bottom",
           conf.int = TRUE,
           linetype = "strata",
           legend.title = "type of operation undergone by the patient",
           xlab = "Months",
           ylab = "Survival probability")
gg
ggsave("KM_OP.png", plot=print(gg))
```

The logrank test:
```{r}
survdiff(Surv(OS, death) ~ OP, data=lungSCC)
```
...
>> ADD INTERPRETATIONS HERE + PROPORTIONAL



*# Part3: Cox Proportional Hazards Regression Model fit - Multivariate Analysis*
