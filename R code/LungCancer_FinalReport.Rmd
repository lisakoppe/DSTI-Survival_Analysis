...
lungSCC <- mutate(lungSCC_raw,
                  ...
                  OP = factor(op_type,levels = c("1", "2", "3", "4","5"),
                              labels = c("lobectomy","pneumonectomy","segmentectomy","LN biopsy","No OP")),
...
NOTE: Gonna make more sense if we change reference group fom emphysema to no remarkable lung disease!!!
...
```{r}
lungSCC_fullmodel_Lo <- glm(death ~ Location + Obst_pn_or_plugging + Necrosis + Underlying_lung + Effusion + Sex + AG + LN_ratio + cT+ cN + cM + pT+ pN+ pM + Smoking_state + FEV1_FVC + CEA + meta , data = lungSCC)
```


Automatic model selection based on AIC
```{r}
lungSCC_multiLo <- step(lungSCC_fullmodel_Death)
````
Interpretation:
After backward selection, we found that the best model to predict death status composes of 9 variables with the converted AIC 483.13 from the original AIC 502.76

```{r}
summary(lungSCC_multiLo)
confint(lungSCC_multiLo)  # 95% CI
```

Define odds ratio of each variable with significant p-value
```{r}
exp(0.367529)
exp(0.157739)
exp(0.181661)
...
1. Underlying lung disease: 
   Patients who have ILA has 40% higher risk of death than patients with emphysema

2. Age: 
   Patients that older than 60 years old have approximately 17% higher risk than patients that younger than 60 years old of every 1 year older
   
3. Size of tumor in central lung cancer patients (cT):
   Patients in criteria T2 (Tumor diameter 2.1-4 cm) have 20% higher risk than patient with criteria T1 while this higher risk is about 40% in patient with T3 and T4
   
   
5. Nearby lymph nodes that are involved in peripheral lung cancer patients (pN);
   Peripheral lung cancer patients whom their cancers spread to lymph nodes in the centre of the chest or under the windpipe brances off to each lung (N2) have approximately 30% higher risk than patients who show no nodal involvement.

6. Smoking history:
   Patients who used to smoke and non smoker have 10% and 20% lower risk than patients who have unknown record about smoking
IMPORTATN! SHOULD CHANGE THE REF GROUP FROM UNKNOWN TO SMOKER (also in multivariate cox) 


7. Metastasis 
   For both type of lung cancer, patients who have cancer spreading outside the lung have approximately 30% higher risk than those without metastasis
...
## Comparision between 2 groups of patients based on Age
...
```
Interpretation: 
1. Among total 90 patients below the age of 60, 26 diead .
2. Among total 308 patients above the age of 60, 146 diead with 63 % Chance of death at 82 months .
...
## Comparision between 2 groups of patients based on Central Tumor Size
...
```
Interpretation: 
1. Among total 73 patients with tumor size T1, 21 diead .
2. Among total 252 patients with tumor size T2, 105 diead .
3. Among total 58 patients with tumor size T3, 37 diead with  24 % chances of death at 39 months.
4. Among total 15 patients with tumor size T4, 9 diead  .


We plot the Kaplan-Meier survival curve:
```{r}
ggsurvplot(fit.KM_cT,
           legend = "bottom",
           conf.int = TRUE,
           linetype = "strata",
           legend.title = "Observation of cT",
           xlab = "Months",
           ylab = "Survival probability")
```
...
## Comparision between 2 groups of patients based on Central Nearby lymph nodes that are involved
```{r}
fit.KM_cN <- survfit(Surv(OS, death)~cN, data=lungSCC)
fit.KM_cN
```
Interpretation: 
1. Among total 284 patients with N0 Nearby lymph nodes that are involved, 114 diead .
2. Among total 68 patients with N1 Nearby lymph nodes that are involved, 27 diead .
3. Among total 39 patients with N2 Nearby lymph nodes that are involved, 27 diead with  25 % chances of death at 38 months.
4. Among total 7 patients with N0 Nearby lymph nodes that are involved, 4 diead with  18 % chances of death at 31 months.


We plot the Kaplan-Meier survival curve:
```{r}
ggsurvplot(fit.KM_cN,
           legend = "bottom",
           conf.int = TRUE,
           linetype = "strata",
           legend.title = "Observation of cN",
           xlab = "Months",
           ylab = "Survival probability")
```
...
## Comparision between 2 groups of patients based on Operation type
```{r}
fit.KM_op_type <- survfit(Surv(OS, death)~op_type, data=lungSCC)
fit.KM_op_type
```
Interpretation: 
1. Among total 325 patients with Type 1 surgery , 131 diead .
2. Among total 54 patients with Type 2 surgery, 27 diead with  39 % chances of death at 82 months.
3. Among total 4 patients with Type 3 surgery, 1 diead.
4. Among total 13 patients with Type 4 surgery, 11 diead with  7 % chances of death at 15 months.
5. Among total 2 patients with No operation , 2 diead with  25 % chances of death at 32 months.


We plot the Kaplan-Meier survival curve:
```{r}
ggsurvplot(fit.KM_op_type,
           legend = "bottom",
           conf.int = TRUE,
           linetype = "strata",
           legend.title = "Observation of Operation type",
           xlab = "Months",
           ylab = "Survival probability")
```
...
The logrank test:
```{r}
survdiff(Surv(OS, death)~op_type, data=lungSCC)
```
Interpretation : 
Logrank test result shows a significant difference in survival between these groups of patients (p= 6e-06).
