...
lungSCC <- mutate(lungSCC_raw,
                  ...
                  Multiplicity = factor(Multiplicity,labels=c("absent","present")),
                  
                  cT = factor(cT,labels=c("T1","T2","T3","T4")),
                 
                  cN = factor(cT,labels=c("N0","N1","N2","N3")),
          
                  Smoking_state = factor(Smoking_state,labels = c("unknown","current-smoker","ex- smoker",
                                                                  "non- smoker")),
...
```{r}
summary(glm(death ~ OP, data = lungcancer_PC, family = "binomial"))
summary(fit)
```
...
```{r}
summary(glm(death ~ Smoking_state, data = lungcancer_PC, family = "binomial"))
summary(fit)
...
```{r}
summary(glm(death ~ cT, data = lungcancer_PC, family = "binomial"))
summary(fit)
```
...
```{r}
summary(glm(death ~ cN, data = lungcancer_PC, family = "binomial"))
summary(fit)
```
...
```{r}
summary(glm(death ~ cM, data = lungcancer_PC, family = "binomial"))
summary(fit)
...
Methods used: Univariate Cox regression
